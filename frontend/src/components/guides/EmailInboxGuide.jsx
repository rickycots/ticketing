import { Mail, MailOpen, AlertTriangle, Star, Info, Reply, Ticket as TicketIcon, Plus } from 'lucide-react'
...
const items = [
  ...
  { n: 3, title: 'Nuova Email', description: 'Apre il form per inviare una mail manualmente da ticketing@ o assistenzatecnica@ ad un cliente.' },
  { n: 4, title: 'Lista email (card)', description: 'Ogni card mostra: icona lettura, direzione (ricevuta/inviata), flag bloccante/rilevante/di-contesto, oggetto, mittente e data.' },
  { n: 5, title: 'Icona lettura', description: 'Cerchio blu pieno (Mail) = non letta. Cerchio grigio vuoto (MailOpen) = già letta.' },
  { n: 6, title: 'Badge direzione', description: 'Viola = Ricevuta, Verde = Inviata.' },
  { n: 7, title: 'Flag rilevanza/blocco', description: '⚠ arancione = Bloccante (ferma il progetto); ⭐ viola = Rilevante; ℹ grigio = Di contesto.' },
  { n: 8, title: 'Pulsanti Rispondi / Rispondi a tutti', description: 'La risposta viene inviata da ticketing@ con tag [TICKET #TK-...] per essere auto-agganciata al thread.' },
  { n: 9, title: 'Crea Ticket da email', description: 'Crea un nuovo ticket usando questa email come primo messaggio. Auto-reply automatica al mittente. Se il mittente non è utente portale del cliente, chiede a chi notificare.' },
  { n: 10, title: 'Assegnazioni', description: 'Associa la mail a un cliente, progetto e attività. Utile per raggruppare in contesto.' },
]
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          ...
          <div className="flex gap-2 items-center flex-wrap text-[10px]">
            ...
            <button className="ml-auto inline-flex items-center gap-1 bg-blue-600 text-white rounded-lg px-2 py-1">
              <Plus size={10} /> Nuova
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-5 gap-2">
          ...
          <div className="col-span-2 relative bg-white rounded-xl border border-gray-200 divide-y divide-gray-100 overflow-hidden">
            <GuideNumber n={4} x={-8} y={6} />
            <div className="p-2 bg-blue-50/50 relative">
              <GuideNumber n={5} x={-3} y={2} />
              <GuideNumber n={6} x={22} y={2} />
              <GuideNumber n={7} x={90} y={2} />
