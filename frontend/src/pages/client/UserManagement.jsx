...
export default function UserManagement() {
  ...
  const [form, setForm] = useState({ nome: '', email: '', password: '', schede_visibili: 'ticket,progetti' })
  ...
  function openForm(user = null) {
    if (user) {
      setEditingUser(user)
      setForm({ nome: user.nome, email: user.email, password: '', schede_visibili: user.schede_visibili })
    } else {
      setEditingUser(null)
      setForm({ nome: '', email: '', password: '', schede_visibili: 'ticket,progetti' })
    }
    setShowForm(true)
  }
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-4">
          <h2 className="text-sm font-semibold mb-3">{editingUser ? 'Modifica Utente' : 'Nuovo Utente'}</h2>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Sezioni Visibili</label>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400 text-sm">Nessun utente</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-gray-500 border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-4 py-3 font-medium">Sezioni</th>
                ...
                return (
                  <tr key={u.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-medium text-gray-900">
                      {u.nome}
                      {isMe && <span className="ml-1.5 text-xs text-blue-500">(tu)</span>}
                    </td>
