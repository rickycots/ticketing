...
router.post('/', authenticateToken, requireAdmin, (req, res) => {
  const { nome, email, password, cambio_password, abilitato_ai, gestione_avanzata, two_factor } = req.body;
  ...
  ).run(nome, email, password_hash, 'tecnico', cambio_password ? 1 : 0, abilitato_ai ? 1 : 0, gestione_avanzata ? 1 : 0, two_factor ? 1 : 0);

  const user = db.prepare('SELECT id, nome, email, ruolo, attivo, cambio_password, abilitato_ai, gestione_avanzata, two_factor, created_at FROM utenti WHERE id = ?').get(result.lastInsertRowid);
  res.status(201).json(user);
});

// PUT /api/users/:id — update user (admin-only)
router.put('/:id', authenticateToken, requireAdmin, (req, res) => {
  const { nome, email, password, attivo, abilitato_ai, gestione_avanzata, two_factor } = req.body;
  ...
  ).run(nome || null, email || null, newHash, attivo !== undefined ? attivo : null, abilitato_ai !== undefined ? (abilitato_ai ? 1 : 0) : null, gestione_avanzata !== undefined ? (gestione_avanzata ? 1 : 0) : null, two_factor !== undefined ? (two_factor ? 1 : 0) : null, req.params.id);

  const updated = db.prepare('SELECT id, nome, email, ruolo, attivo, cambio_password, abilitato_ai, gestione_avanzata, two_factor, created_at FROM utenti WHERE id = ?').get(req.params.id);
  res.json(updated);
});
